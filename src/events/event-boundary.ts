...
import { FederatedMouseEvent } from './federated-mouse-event';
import { FederatedPointerEvent } from './federated-pointer-event';
import { FederatedWheelEvent } from './federated-wheel-event';
import type { Shape } from '../shapes';
import type {
    EmitterListeners,
    Picker,
    TrackingData,
} from './event-boundary-type';
import type { FederatedEvent } from './federated-event';
import type { Cursor, FederatedEventTarget } from './federated-event-target';
...
export class EventBoundary {
    ...
    constructor(rootTarget?: Shape) {
        ...
        // this ensures that reassignment of these functions won't be broken and unlinked from the original object
    ...
    propagate(e: FederatedEvent, type?: string): void {
        ...
        // Capturing phase
        // events are passed down from the top outermost element
        ...
        // Bubbling phase
        // events are passed up from the innermost element (reverse passing)
    ...
    all(
        e: FederatedEvent,
        type?: string | string[],
        targets = this._allInteractiveElements
    ): void {
    ...
    protected mapPointerMove(from: FederatedEvent): void {
        ...
        if (trackingData.overTargets?.length > 0 && outTarget !== e.target) {
            ...
            const outType =
                from.type === 'mousemove' ? 'mouseout' : 'pointerout';
        ...
        if (outTarget !== e.target) {
            ...
            const overType =
                from.type === 'mousemove' ? 'mouseover' : 'pointerover';
        ...
        if (isMouse) {
            this.moveOnAll
                ? allMethods.splice(1, 0, 'mousemove')
    ...
    protected mapPointerOut(from: FederatedEvent): void {
        ...
        if (trackingData.overTargets) {
            ...
            while (
                leaveEvent.target &&
                leaveEvent.target !== this.rootTarget.parent
            ) {
                leaveEvent.currentTarget = leaveEvent.target;
     ...
     */
    protected copyWheelData(
        from: FederatedWheelEvent,
        to: FederatedWheelEvent
    ): void {
    ...
    private _notifyListeners(e: FederatedEvent, type: string): void {
        const listeners = (
            (e.currentTarget as any)._events as EmitterListeners
        )[type];
