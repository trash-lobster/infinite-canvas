...
import type { EventBoundary } from './event-boundary';
import type { FederatedEventTarget } from './federated-event-target';
  ...
  implements UIEvent
{
    ...
    /** Flags whether this event can be canceled using {@link FederatedEvent.preventDefault}. */
    ...
    composedPath(): FederatedEventTarget[] {
        // Find the propagation path if it isn't cached or if the target has changed since
        // the last evaluation.
        if (
            this.manager &&
